            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.trailing) {
                    VStack(alignment: .trailing, spacing: 1) {
                        Zahl(state: context.state, groesse: 26)
                        if let seit = context.state.begonnen {
                            Text(timerInterval: seit...Date.distantFuture, countsDown: false)
                    ...
                    /* Die runden Ecken des aufgeklappten Bereichs schneiden
                       sonst in die Schrift. Ein paar Punkte Abstand kosten
                       nichts und sind der Unterschied zwischen lesbar und
                       angeknabbert. */
                    .padding(.trailing, 4)
                }
                ...
                /* Eine Zeit, nicht ein Zaehlerstand. Hier stand "3/20" — die
                   Satzzahl sieht man beim Aufklappen ohnehin, und im
                   zugeklappten Streifen ist die Frage eine andere: laeuft die
                   Pause noch, und wie lange bin ich schon dabei. */
                if let bis = context.state.pauseBis {
                    Text(timerInterval: Date()...bis, countsDown: true)
                        ...
                        .foregroundStyle(Color.amber)
                        .frame(maxWidth: 44)
                } else if let seit = context.state.begonnen {
                    Text(timerInterval: seit...Date.distantFuture, countsDown: false)
                        .font(.system(size: 13, weight: .semibold, design: .rounded))
                        .monospacedDigit()
                        .foregroundStyle(Color.knochen)
                ...
                /* Der kleinste Zustand hat Platz fuer ein Zeichen — aber ein
                   Symbol allein sagt nur "irgendetwas laeuft". Waehrend der
                   Pause zeigt er deshalb die Restsekunden, sonst das Symbol. */
                if let bis = context.state.pauseBis {
                    Text(timerInterval: Date()...bis, countsDown: true)
