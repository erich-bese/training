...
(function () {
  ...
  /* --------------------------------------------------------- sound ------- */
  /* Der Pausenton lief bisher ueber Web Audio und war auf dem Telefon nicht
     zu hoeren. Er scheiterte an drei Bedingungen, von denen bei einem
     ablaufenden Zeitgeber keine erfuellt ist: Web Audio will eine
     Nutzergeste, der Kontext schlaeft ein, sobald die Ansicht nicht vorn ist,
     und ueber einen Wechsel in den Hintergrund kommt er ohnehin nicht.
     Nativ gilt nur die Kategorie der Audiositzung, und die steht auf
     `.playback` — damit klingt er auch bei stumm geschaltetem Telefon, was
     beim Training auf der Bank der Normalfall ist.

     Die Seite prueft auf diese Funktion; fehlt sie, bleibt es bei Web Audio,
     damit die App im Browser unveraendert weiterlaeuft. */
  window.__training_sound = function (art) {
  ...
  window.__training_openHealth = function () {
    post({ cmd: "openhealth" });
  };
