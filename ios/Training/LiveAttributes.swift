...
struct TrainingAttributes: ActivityAttributes {
    ...
    struct ContentState: Codable, Hashable {
        ...
        var pauseVon: Date?
        /// When the session itself started.
        ///
        /// Same reasoning as `pauseBis`: a date, so the system counts the
        /// elapsed time on its own. `nil` means the session is open but not
        /// started — then there is nothing to count and the field stays blank
