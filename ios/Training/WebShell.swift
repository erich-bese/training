...
final class WebShell: UIViewController {
    ...
    /// Called when the app returns to the front.
    ///
    /// The session goes inactive in the background and does not come back on
    /// its own. Setting it up once at launch was not enough: after the first
    /// switch away the rest timer stayed silent for the rest of the session,
    /// which is exactly when the sound matters.
    func reactivateAudio() {
        configureAudio()
    }
